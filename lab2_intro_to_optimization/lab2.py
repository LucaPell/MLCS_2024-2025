...
def GD(x_0,y_0,eta,num_epoch,fun):
    ...
    if np.size(eta) == 1: #! il controllo lo faccio su numpy per non dovere fargli definire in torch i singoli valori del learning rate ma si puo cambiare
...
def plot_3D(x, y, fun):
    with torch.no_grad():
        ...
        # Customize the layout to improve plot aesthetics
         # Dark theme settings
    fig.update_layout(
        title="3D Surface Plot of f(x, y) = x² + y²",  # Add a title
...
def plot_2D_GD_interactive(method, x, y, x_min, y_min, fun, x_0, y_0, eta, num_epochs):
    ...
    with torch.inference_mode():
        ...
        contour = ax.contourf(x_mesh, y_mesh, fun(x_mesh, y_mesh))
